...
static void dump_free_variables(Context* ctx, CFNode* cfnode, int depth) {
    ...
    if (true) {
        ...
        while (dict_iter(ctx->current_scope->bound_set, &i, &bound_var, NULL)) {
            if (prev) {
                debugvv_print(", ");
            }
...
static CFNodeVariables* visit_domtree(Context* ctx, CFNode* cfnode, int depth, CFNodeVariables* parent) {
    ...
    for (size_t i = 0; i < entries_count_list(cfnode->dominates); i++) {
        ...
        CFNodeVariables* child_variables = visit_domtree(ctx, child, depth + 1, ctx->current_scope);
        size_t j = 0;
        const Node* free_var;
    ...
    // dump_free_variables(ctx, cfnode, depth);
