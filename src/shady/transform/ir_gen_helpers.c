...
Nodes gen_primop_c(BodyBuilder* bb, Op op, size_t operands_count, const Node* operands[]) {
    return gen_primop(bb, op, shd_empty(bb->arena), shd_nodes(bb->arena, operands_count, operands));
}
...
const Node* gen_get_stack_size(BodyBuilder* bb) {
    return shd_first(bind_instruction(bb, get_stack_size(bb->arena, (GetStackSize) { .mem = bb_mem(bb) })));
}
...
const Node* gen_reinterpret_cast(BodyBuilder* bb, const Type* dst, const Node* src) {
    assert(is_type(dst));
    return prim_op(bb->arena, (PrimOp) { .op = reinterpret_op, .operands = shd_singleton(src), .type_arguments = shd_singleton(dst)});
}

const Node* gen_conversion(BodyBuilder* bb, const Type* dst, const Node* src) {
    assert(is_type(dst));
    return prim_op(bb->arena, (PrimOp) { .op = convert_op, .operands = shd_singleton(src), .type_arguments = shd_singleton(dst)});
}
...
const Node* gen_stack_alloc(BodyBuilder* bb, const Type* type) {
    return shd_first(bind_instruction(bb, stack_alloc(bb->arena, (StackAlloc) { .type = type, .mem = bb_mem(bb) })));
}

const Node* gen_local_alloc(BodyBuilder* bb, const Type* type) {
    return shd_first(bind_instruction(bb, local_alloc(bb->arena, (LocalAlloc) { .type = type, .mem = bb_mem(bb) })));
}
...
const Node* gen_load(BodyBuilder* bb, const Node* ptr) {
    return shd_first(bind_instruction(bb, load(bb->arena, (Load) { .ptr = ptr, .mem = bb_mem(bb) })));
}
...
const Node* convert_int_zero_extend(BodyBuilder* bb, const Type* dst_type, const Node* src) {
    ...
    val = gen_primop_e(bb, reinterpret_op, shd_singleton(
        int_type(bb->arena, (Int) { .width = src_type->payload.int_type.width, .is_signed = false })), shd_singleton(val));
    val = gen_primop_e(bb, convert_op, shd_singleton(
        int_type(bb->arena, (Int) { .width = dst_type->payload.int_type.width, .is_signed = false })), shd_singleton(val));
    val = gen_primop_e(bb, reinterpret_op, shd_singleton(dst_type), shd_singleton(val));
    return val;
}
...
const Node* convert_int_sign_extend(BodyBuilder* bb, const Type* dst_type,  const Node* src) {
    ...
    const Node* val = src;
    val = gen_primop_e(bb, reinterpret_op, shd_singleton(
        int_type(bb->arena, (Int) { .width = src_type->payload.int_type.width, .is_signed = true })), shd_singleton(val));
    val = gen_primop_e(bb, convert_op, shd_singleton(
        int_type(bb->arena, (Int) { .width = dst_type->payload.int_type.width, .is_signed = true })), shd_singleton(val));
    val = gen_primop_e(bb, reinterpret_op, shd_singleton(dst_type), shd_singleton(val));
    return val;
}
