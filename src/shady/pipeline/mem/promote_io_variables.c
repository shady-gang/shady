...
static const Node* promote_to_physical(Context* ctx, ShdScope scope, const Node* io) {
    ...
    Node* phy = global_var(m, shd_empty(a), io->payload.global_variable.type, shd_fmt_string_irarena(a, "%s_physical", io->payload.global_variable.name), scope >= ShdScopeInvocation ? AsPrivate : AsSubgroup, false);
