...
void shd_bld_stack_push_value(BodyBuilder* bb, const Node* value) {
    shd_bld_add_instruction_extract(bb, push_stack(shd_get_bb_arena(bb), (PushStack) { .value = value, .mem = shd_bld_mem(bb) }));
}
...
const Node* shd_bld_get_stack_size(BodyBuilder* bb) {
    return shd_first(shd_bld_add_instruction_extract(bb, get_stack_size(shd_get_bb_arena(bb), (GetStackSize) { .mem = shd_bld_mem(bb) })));
}

void shd_bld_set_stack_size(BodyBuilder* bb, const Node* new_size) {
    shd_bld_add_instruction_extract(bb, set_stack_size(shd_get_bb_arena(bb), (SetStackSize) { .value = new_size, .mem = shd_bld_mem(bb) }));
}
