    ...
    if (is_loop_entry) {
        ...
        if (exiting_nodes_count > 0) {
            ...
            for (size_t i = 0; i < exiting_nodes_count; i++) {
                ...
                shd_set_debug_name(wrapper, shd_fmt_string_irarena(a, "exit_wrapper_%d", i));
            ...
            for (size_t j = 0; j < inner_control_results.count; j++) {
                inner_control_results = shd_change_node_at_index(a, inner_control_results, j, scope_cast_helper(a, shd_get_qualified_type_scope(nparams.nodes[j]->type), inner_control_results.nodes[j]));
            }
            ...
            for (size_t i = 0; i < exiting_nodes_count; i++) {
                ...
                shd_set_debug_name(exit_bb, shd_fmt_string_irarena(a, "exit_recover_values_%s", shd_get_node_name_safe(exiting_node->node)));
                ...
                for (size_t j = 0; j < exits[i].params_count; j++) {
                    ...
                    recovered_args[j] = scope_cast_helper(a, exits[i].params[j].scope, recovered_args[j]);
                }
...
static const Node* process_node(Context* ctx, const Node* node) {
    ...
    switch (node->tag) {
        ...
        case Branch_TAG: {
            ...
            shd_set_debug_name(pre_join, shd_fmt_string_irarena(a, "merge_%s_%s", shd_get_node_name_safe(ctx->current_abstraction), shd_get_node_name_safe(post_dominator)));
            ...
            // make sure what was uniform still is
            for (size_t j = 0; j < old_params.count; j++) {
                results = shd_change_node_at_index(a, results, j, scope_cast_helper(a, param_scope[j], results.nodes[j]));
            }
...
Module* shd_pass_reconvergence_heuristics(SHADY_UNUSED const CompilerConfig* config, Module* src) {
