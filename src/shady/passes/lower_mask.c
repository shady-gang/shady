...
const Node* process_let(Context* ctx, const Node* old_let) {
    ...
    if (old_instruction->tag == PrimOp_TAG) {
        ...
        switch(op) {
            case empty_mask_op: {
                ...
                instruction = quote(dst_arena, zero);
                break;
            }
            case mask_is_thread_active_op: {
                ...
                return finish_body(bb, let(dst_arena, false, quote(dst_arena, acc), tail));
            }
            ...
            case subgroup_ballot_op: {
                ...
                return finish_body(bb, let(dst_arena, false, quote(dst_arena, result), tail));
            }
            default: break;
        }
    }
    ...
    return let(dst_arena, false, instruction, tail);
}
