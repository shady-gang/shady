...
static const Node* process(Context* ctx, const Node* old) {
    ...
    switch (old->tag) {
        case CopyBytes_TAG: {
            ...
            const Type* word_type = int_type(a, (Int) { .is_signed = false, .width = aconfig->rules.memory.word_size });
            ...
            dst_addr = shd_bld_bitcast(bb, dst_addr_type, dst_addr);
            ...
            const Node* next_index = prim_op_helper(a, add_op, mk_nodes(a, index, shd_uint32_literal(a, 1)));
            ...
            Node* false_case = basic_block_helper(a, shd_empty(a));
            ...
            shd_set_abstraction_body(loop_case, shd_bld_finish(loop_bb, branch(a, (Branch) {
                ...
                .condition = prim_op_helper(a, lt_op, mk_nodes(a, next_index, num_in_words)),
        ...
        case FillBytes_TAG: {
            ...
            size_t req_value__size = int_size_in_bytes(aconfig->rules.memory.word_size);
            ...
            while (value_size < req_value__size) {
                const Node* shifted = prim_op_helper(a, lshift_op, mk_nodes(a, src_value, int_literal_helper(a, aconfig->rules.memory.word_size, false, value_size * 8)));
