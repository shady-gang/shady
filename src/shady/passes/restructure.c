...
static const Node* handle_bb_callsite(Context* ctx, Jump jump, const Node* mem, const Node* exit) {
    ...
    if (prior_encounter) {
        ...
        Node* inner_exit_ladder_bb = basic_block(a, shd_empty(a), unique_name(a, "exit_ladder_inline_me"));
...
static const Node* structure(Context* ctx, const Node* body, const Node* exit) {
    ...
    switch (is_terminator(body)) {
        ...
        case Control_TAG: {
            ...
            shd_set_abstraction_body(tail, finish_body(bb_tail, jump_helper(a, bb_mem(bb_tail), exit, shd_empty(a))));
...
static const Node* process(Context* ctx, const Node* node) {
    ...
    if (node->tag == Function_TAG) {
        ...
        if (is_builtin || !node->payload.fun.body || shd_lookup_annotation(node, "Structured") || setjmp(ctx2.bail)) {
            ...
            shd_set_abstraction_body(new, finish_body(bb, structure(&ctx2, get_abstraction_body(node), make_unreachable_case(a))));
            is_leaf = true;
