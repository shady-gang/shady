...
static void add_bounds_check(IrArena* a, BodyBuilder* bb, const Node* i, const Node* max) {
    ...
    shd_bld_if(bb, shd_empty(a), prim_op_helper(a, gte_op, mk_nodes(a, i, max)), out_of_bounds_case, NULL);
}
...
static const Node* process(Context* ctx, const Node* node) {
    ...
    switch (node->tag) {
        case BuiltinRef_TAG: {
            ShdBuiltin b = node->payload.builtin_ref.builtin;
            switch (b) {
                case ShdBuiltinSubgroupId:
                case ShdBuiltinWorkgroupId:
                case ShdBuiltinGlobalInvocationId:
                case ShdBuiltinLocalInvocationId: {
        ...
        case Function_TAG: {
            ...
            if (epa && strcmp(shd_get_annotation_string_payload(epa), "Compute") == 0) {
                ...
                const Node* num_workgroups_var = shd_rewrite_node(&ctx->rewriter, shd_get_or_create_builtin(ctx->rewriter.src_module, ShdBuiltinNumWorkgroups));
                ...
                assert(aconfig->specializations.workgroup_size[0] && aconfig->specializations.workgroup_size[1] && aconfig->specializations.workgroup_size[2]);
                num_subgroups[0] = aconfig->specializations.workgroup_size[0] / ctx->target_config->subgroup_size;
                num_subgroups[1] = aconfig->specializations.workgroup_size[1];
                num_subgroups[2] = aconfig->specializations.workgroup_size[2];
                ...
                shd_bld_store(bb2, shd_rewrite_node(&ctx->rewriter, shd_get_or_create_builtin(ctx->rewriter.src_module, ShdBuiltinWorkgroupId)), composite_helper(a, vector_type(a, (VectorType) { .element_type = shd_uint32_type(a), .width = 3 }), mk_nodes(a, workgroup_id[0], workgroup_id[1], workgroup_id[2])));
                // write the local ID
                const Node* local_id[3];
                // local_id[0] = SUBGROUP_SIZE * subgroup_id[0] + subgroup_local_id
                local_id[0] = prim_op_helper(a, add_op, mk_nodes(a, prim_op_helper(a, mul_op, mk_nodes(a, shd_uint32_literal(a, ctx->target_config->subgroup_size), subgroup_id[0])), shd_bld_builtin_load(m, bb, ShdBuiltinSubgroupLocalInvocationId)));
                local_id[1] = subgroup_id[1];
                local_id[2] = subgroup_id[2];
                shd_bld_store(bb2, shd_rewrite_node(&ctx->rewriter, shd_get_or_create_builtin(ctx->rewriter.src_module, ShdBuiltinLocalInvocationId)), composite_helper(a, vector_type(a, (VectorType) { .element_type = shd_uint32_type(a), .width = 3 }), mk_nodes(a, local_id[0], local_id[1], local_id[2])));
                // write the global ID
                const Node* global_id[3];
                for (int dim = 0; dim < 3; dim++)
                    global_id[dim] = prim_op_helper(a, add_op, mk_nodes(a, prim_op_helper(a, mul_op, mk_nodes(a, shd_uint32_literal(a, aconfig->specializations.workgroup_size[dim]), workgroup_id[dim])), local_id[dim]));
                ...
                for (unsigned scope = 1; scope < 2; scope--) {
                    ...
                    for (unsigned dim = 2; dim < 3; dim--) {
                        ...
                        shd_set_abstraction_body(loop_body, shd_bld_finish(body_bb, merge_continue(a, (MergeContinue) {
                            .args = shd_singleton(prim_op_helper(a, add_op, mk_nodes(a, params[dim], shd_uint32_literal(a, 1)))),
        ...
        case Load_TAG: {
            ...
            if (ptr == shd_get_or_create_builtin(ctx->rewriter.src_module, ShdBuiltinSubgroupId)) {
                ...
                loaded = scope_cast_helper(a, shd_get_builtin_scope(ShdBuiltinsCount), loaded);
                //loaded = prim_op_helper(a, subgroup_assume_uniform_op, shd_empty(a), shd_singleton(loaded));
                return shd_bld_to_instr_yield_values(bb, shd_singleton(loaded));
            }
        }
        default: break;
    }
...
Module* shd_pass_lower_workgroups(SHADY_UNUSED const CompilerConfig* config, Module* src) {
