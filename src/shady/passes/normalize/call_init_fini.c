    ...
    switch (old->tag) {
        case Function_TAG: {
            ...
            if (entry_pt) {
                ...
                Node* new = function_helper(m, shd_recreate_params(r, get_abstraction_params(old)), shd_fmt_string_irarena(a, "%s_", payload.name), mk_nodes(a, annotation_helper(a, "Leaf")), shd_rewrite_nodes(r, payload.return_types));
                ...
                Node* wrapper = function_helper(m, wrapper_params, payload.name, shd_rewrite_nodes(r, payload.annotations), shd_rewrite_nodes(r, payload.return_types));
