    ...
    switch (element_type->tag) {
        ...
        case Int_TAG: ser_int: {
            ...
            for (size_t byte = 0; byte < length_in_bytes; byte += word_size_in_bytes) {
                const Node* word = gen_load(bb, gen_primop_ce(bb, lea_op, 3, (const Node* []) {arr, NULL, offset}));
    ...
    switch (element_type->tag) {
        ...
        case Int_TAG: des_int: {
            ...
            for (size_t byte = 0; byte < length_in_bytes; byte += word_size_in_bytes) {
                ...
                if (needs_patch) {
                    original_word = gen_load(bb, gen_primop_ce(bb, lea_op, 3, (const Node* []) {arr, NULL, offset}));
                    error_print("TODO");
                    error_die();
                    // word = gen_conversion(bb, int_type(a, (Int) { .width = element_type->payload.int_type.width, .is_signed = false }), word); // widen/truncate the word we just loaded
                }*/
                ...
                gen_store(bb, gen_primop_ce(bb, lea_op, 3, (const Node* []) {arr, NULL, offset}), word);
