...
static const Node* gen_serdes_fn(Context* ctx, const Type* element_type, bool uniform_address, bool ser, AddressSpace as) {
    ...
    String name = format_string(a->arena, "generated_%s_as%d_%s_%s", ser ? "store" : "load", as, uniform_address ? "uniform" : "varying", name_type_safe(a, element_type));
...
static void construct_emulated_memory_array(Context* ctx, AddressSpace as, AddressSpace logical_as) {
    IrArena* a = ctx->rewriter.dst_arena;
    Module* m = ctx->rewriter.dst_module;
    String as_name = format_string(a->arena, "as_%d", as);
    Nodes annotations = singleton(annotation(a, (Annotation) { .name = "Generated" }));

    Node* global_struct_t = nominal_type(m, annotations, format_string(a->arena, "globals_physical_%s_t", as_name));
    ...
    Node* words_array = global_var(m, annotations, words_array_type, format_string(a->arena, "addressable_word_memory_%s", as_name), logical_as);
