...
static const Node* process(Context* ctx, const Node* node) {
    ...
    switch (node->tag) {
        case Jump_TAG: {
            ...
            Node* new_block = basic_block_helper(a, shd_empty(a));
...
Module* shd_pass_remove_critical_edges(SHADY_UNUSED const CompilerConfig* config, SHADY_UNUSED const void* unused, Module* src) {
