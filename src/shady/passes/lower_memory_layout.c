        ...
        case PrimOp_TAG: {
            switch (old->payload.prim_op.op) {
                case size_of_op: {
                    ...
                    return quote_helper(a, singleton(int_literal(a, (IntLiteral) {.width = a->config.memory.ptr_size, .is_signed = false, .value.u64 = layout.size_in_bytes})));
                }
                case align_of_op: {
                    const Type* t = rewrite_node(&ctx->rewriter, first(old->payload.prim_op.type_arguments));
                    TypeMemLayout layout = get_mem_layout(a, t);
                    return quote_helper(a, singleton(int_literal(a, (IntLiteral) {.width = a->config.memory.ptr_size, .is_signed = false, .value.u64 = layout.alignment_in_bytes})));
                }
                case offset_of_op: {
                    const Type* t = rewrite_node(&ctx->rewriter, first(old->payload.prim_op.type_arguments));
