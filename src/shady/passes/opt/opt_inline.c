        ...
        case Call_TAG: {
            ...
            if (ocallee->tag == Function_TAG) {
                ...
                if (get_inlining_heuristic(ctx->config, fn_node).can_be_inlined && is_call_potentially_inlineable(ctx->old_fun, ocallee)) {
                    ...
                    const Node* join_point = param_helper(a, shd_as_qualified_type(jp_type, true), shd_format_string_arena(a->arena, "inlined_return_%s", shd_get_abstraction_name(ocallee)));
