...
const struct Node* rewrite_node(struct Rewriter* rewriter, const struct Node* node) { return rewriter->rewrite_fn(rewriter, node); }
...
const struct Node* recreate_node_identity(struct Rewriter* rewriter, const struct Node* node) {
    ...
    switch (node->tag) {
        ...
        case ContType_TAG:      return cont_type(rewriter->dst_arena, (struct ContType) { .param_types = rewrite_nodes(rewriter, node->payload.cont_type.param_types) });
