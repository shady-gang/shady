...
static const Type* get_type_for_signature(Context* ctx, const Node* fnt) {
    ...
    for (size_t i = 0; i < param_types.count; i++) {
        members[j++] = shd_get_unqualified_type(param_types.nodes[i]);
    }
    for (size_t i = 0; i < ret_types.count; i++) {
        members[j++] = shd_get_unqualified_type(ret_types.nodes[i]);
    }
...
static Nodes rewrite_call(Context* ctx, BodyBuilder* bb, const Node* ocallee, const Nodes old_args) {
    ...
    const Node* var = get_caller_var_for_signature(ctx, shd_get_unqualified_type(ocallee)->type);
    for (size_t i = 0; i < old_args.count; i++) {
        const Node* arg = shd_rewrite_node(r, old_args.nodes[0]);
        shd_bld_store(bb, ptr_array_element_offset_helper(a, var, shd_uint32_literal(a, i)), arg);
    }

    shd_bld_add_instruction(bb, ext_instr_helper(a, shd_bld_mem(bb), unit_type(a), "spirv.core", SpvOpExecuteCallableKHR, mk_nodes(a, shd_rewrite_node(r, ocallee), var)));

    size_t num_results = ofnt->payload.fn_type.return_types.count;
    LARRAY(const Node*, results, num_results);
    for (size_t i = 0; i < num_results; i++) {
        results[i] = shd_bld_load(bb, ptr_array_element_offset_helper(a, var, shd_uint32_literal(a, old_args.count + i)));
    }
    return shd_nodes(a, old_args.count, results);
}
...
static const Node* process(Context* ctx, const Node* node) {
    ...
    switch (node->tag) {
        ...
        case Function_TAG: {
            ...
            const Node* var = get_callee_var_for_signature(ctx, shd_get_unqualified_type(node->type));
            ...
            for (size_t i = 0; i < params.count; i++) {
                args[i] = shd_bld_load(bb, ptr_array_element_offset_helper(a, var, shd_uint32_literal(a, i)));
            }
            Nodes results = shd_bld_call(bb, new, shd_nodes(a, params.count, args));
            for (size_t i = 0; i < results.count; i++) {
                shd_bld_store(bb, ptr_array_element_offset_helper(a, var, shd_uint32_literal(a, params.count + i)), results.nodes[i]);
            }
